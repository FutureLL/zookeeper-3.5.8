...
public class ZooKeeper implements AutoCloseable {
    ...
    /**
     * 储存已经注册的 Watch,查找事件触发的 Watch
     *
     * 管理观察者并处理由 ClientCnxn 对象生成的事件
     * Manage watchers & handle events generated by the ClientCnxn object.
     *
     * 我们将其实现为一个嵌套的 ZooKeeper 类,这样公共方法就不会作为 ZooKeeper 客户端API的一部分被公开。
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher,
        ...
        // 创建 ConnectStringParser 对象,用于存储解析包装过的连接地址信息 connectString == host
        ConnectStringParser connectStringParser = new ConnectStringParser(connectString);
        /**
    ...
    public String create(final String path, byte data[], List<ACL> acl, CreateMode createMode) throws KeeperException, InterruptedException {
        ...
        // 如何设置了 chrootPath,那么根据 chrootPath 和 clientPath 形成最终节点在服务端的路径
        final String serverPath = prependChroot(clientPath);

        // 构建请求头
        RequestHeader h = new RequestHeader();
        // 设置请求头的 type 信息,请求头不同的 type 信息表上不同的请求,将来服务端就是根据这个 type 信息来决定做如何处理
